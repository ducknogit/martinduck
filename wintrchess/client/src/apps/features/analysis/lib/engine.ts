...
class Engine {
    ...
    setLineCount(lines: number) {
        // Enforce minimum MultiPV of 2 for accurate move classification
        // (Brilliant/Critical moves require second-best line for comparison)
        const effectiveLines = Math.max(lines, 2);
        this.setOption("MultiPV", effectiveLines.toString());
    ...
    async evaluate(options: {
        ...
        await this.consumeLogs(
            ...
            log => {
                ...
                const existingLineIndex = engineLines.findIndex(
                    line => line.index == index
                );

                if (existingLineIndex !== -1) {
                    engineLines[existingLineIndex] = newEngineLine;
                } else {
