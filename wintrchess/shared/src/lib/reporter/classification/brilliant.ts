...
export function considerBrilliantClassification(
    ...
    if (!isMoveCriticalCandidate(previous, current)) {
        return false;
    }

    // Require MultiPV ≥ 2 data for accurate classification
    if (!previous.secondSubjectiveEvaluation) {
        console.warn(
            "Cannot classify as Brilliant: missing second-best move data. " +
            "Ensure engine is configured with MultiPV ≥ 2."
        );
        return false;
    }
    ...
    // Allow brilliant even if there are counterthreats (relaxed criteria)
